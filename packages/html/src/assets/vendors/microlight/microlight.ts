      ...
      while (
        ...
        if (
          ...
          if (token) {
            ...
            el[appendChild]((node = _document.createElement('span'))).setAttribute(
              'style',
              [
                // 0: not formatted
                '',
                // 1: keywords
                textShadow + _0px_0px + 9 + pxColor + alpha * 0.7 + '),' + _0px_0px + 2 + pxColor + alpha * 0.4 + brace,
                // 2: punctuation
                opacity +
                  6 +
                  textShadow +
